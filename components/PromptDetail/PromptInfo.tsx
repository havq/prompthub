    ...
    return (
        <div className="p-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 mb-6 border border-gray-200 dark:border-gray-700">
                <p className="text-gray-800 dark:text-gray-200 whitespace-pre-wrap text-sm leading-relaxed select-text font-mono">
                    {isLongText && !isExpanded ? `${text.substring(0, TEXT_LIMIT)}...` : text}
                </p>
                {isLongText && (
                    <button 
                        onClick={() => setIsExpanded(!isExpanded)} 
                        className="mt-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline focus:outline-none"
                    >
