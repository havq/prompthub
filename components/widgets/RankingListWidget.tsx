        ...
        const fetchData = async () => {
            ...
            try {
                const sortByMap: Record<string, string> = {
                    'views': 'views',
                    'favorites': 'rating', // 'favorites' count sort isn't standard in prompts API, use 'rating' as best proxy for popularity
                    'remixes': 'remixes'
                };
                const sortParam = sortByMap[data.dataSource || 'views'] || 'views';
                
                const res = await getPrompts({ page: 1, limit: data.limit || 5, sortBy: sortParam as any });
