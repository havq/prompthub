...
const PromptBasicDetails: React.FC<PromptBasicDetailsProps> = ({
    ...
    return (
        <>
            ...
            <div>
                <label htmlFor="prompt-text" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('admin.promptForm.promptText')}</label>
                <textarea id="prompt-text" rows={12} value={text} onChange={(e) => setText(e.target.value)}
            ...
            <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    ...
                    <button type="button" onClick={onSuggestTags} disabled={isSuggestingTags || !text.trim()} className="bg-indigo-100 hover:bg-indigo-200 dark:bg-indigo-900/50 dark:hover:bg-indigo-800/80 text-indigo-700 dark:text-indigo-300 font-semibold py-2 px-4 rounded-md transition-colors text-sm w-44 flex justify-center">
