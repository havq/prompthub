...
import { PostCategoryWithCount } from '../../utils/types';
import { useLanguage } from '../../context/LanguageContext';
import { addPostCategory, updatePostCategory } from '../../services/api';
...
const AdminPostCategories: React.FC<AdminPostCategoriesProps> = ({ categories, onDelete, onRefresh }) => {
    const { t } = useLanguage();
    ...
    const sortedCategories = useMemo(() => {
        ...
        const result: (PostCategoryWithCount & { depth: number })[] = [];
    ...
    const handleAddCategory = async (e: React.FormEvent) => {
        ...
        setIsLoading('add');
        try {
            await addPostCategory({ name: newCategoryName, parentId: newCategoryParent || undefined });
            setNewCategoryName('');
            setNewCategoryParent('');
            onRefresh();
        } catch (error) { console.error("Failed to add post category:", error); } 
    ...
    const handleSaveCategory = async () => {
        ...
        try {
            await updatePostCategory({ id: editingCategoryId, name: editedCategoryName, parentId: editedCategoryParent || undefined });
            setEditingCategoryId(null);
            setEditedCategoryName('');
            setEditedCategoryParent('');
            onRefresh();
        } catch (error) { console.error("Failed to save post category:", error); } 
        finally { setIsLoading(null); }
    };

    const startEditing = (category: PostCategoryWithCount) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-2xl mx-auto">
            <h3 className="text-xl font-semibold mb-4">Post Categories</h3>
                        ...
                        {editingCategoryId === category.id ? ( isLoading === `save_${category.id}` ? (<div className="flex-grow flex justify-center"><Spinner size="sm" /></div>) : (
                            ...
                            <div className="flex-grow flex items-center justify-between">
                                <div className="flex items-center" style={{ paddingLeft: `${category.depth * 30}px` }}>
                                    ...
                                    <span className="text-xs text-gray-500 dark:text-gray-400 ml-2 bg-gray-200 dark:bg-gray-600 px-1.5 py-0.5 rounded-full">{category.postCount || 0}</span>
                                </div>
