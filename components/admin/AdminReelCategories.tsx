...
interface AdminReelCategoriesProps {
    categories: ReelCategoryWithCount[];
    onDelete: (id: string) => void;
    onRefresh: () => void;
}

const AdminReelCategories: React.FC<AdminReelCategoriesProps> = ({ categories, onDelete, onRefresh }) => {
    const { t } = useLanguage();
    ...
    const sortedCategories = useMemo(() => {
        ...
        const result: (ReelCategoryWithCount & { depth: number })[] = [];
    ...
    const handleAddCategory = async (e: React.FormEvent) => {
        ...
        setIsLoading('add');
        try {
            await addReelCategory({ name: newCategoryName, parentId: newCategoryParent || undefined });
            setNewCategoryName('');
            setNewCategoryParent('');
            onRefresh();
        } catch (error) { console.error("Failed to add reel category:", error); } 
    ...
    const handleSaveCategory = async () => {
        ...
        try {
            await updateReelCategory({ id: editingCategoryId, name: editedCategoryName, parentId: editedCategoryParent || undefined });
            setEditingCategoryId(null);
            setEditedCategoryName('');
            setEditedCategoryParent('');
            onRefresh();
        } catch (error) { console.error("Failed to save reel category:", error); } 
        finally { setIsLoading(null); }
    };

    const startEditing = (category: ReelCategoryWithCount) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-2xl mx-auto">
            <h3 className="text-xl font-semibold mb-4">Reel Categories</h3>
                        ...
                        {editingCategoryId === category.id ? ( isLoading === `save_${category.id}` ? (<div className="flex-grow flex justify-center"><Spinner size="sm" /></div>) : (
                            ...
                            <div className="flex-grow flex items-center justify-between">
                                <div className="flex items-center" style={{ paddingLeft: `${category.depth * 30}px` }}>
                                    ...
                                    <span className="text-xs text-gray-500 dark:text-gray-400 ml-2 bg-gray-200 dark:bg-gray-600 px-1.5 py-0.5 rounded-full">{category.reelCount || 0}</span>
                                </div>
...
export default AdminReelCategories;
