...
const RegisterPage: React.FC = () => {
  ...
  if (!registrationEnabled) {
    return (
        <div className="max-w-md mx-auto mt-10 text-center bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 space-y-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('register.title')}</h2>
            <p className="mt-4 text-red-500 dark:text-red-400">{t('register.disabled')}</p>
        </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
        return setError(t('register.error.passwordLength'));
    }
    setError('');
    setIsLoading(true);
    ...
    try {
      if (recaptchaSettings?.enabled) {
        ...
        if (recaptchaSettings.version === 'v3') {
          ...
          token = await window.grecaptcha.execute(recaptchaSettings.v3SiteKey, { action: 'register' });
        }
        if (!token) {
  ...
  return (
    <div className="max-w-md mx-auto mt-10">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 space-y-6">
        <h2 className="text-2xl font-bold text-center text-gray-900 dark:text-white">{t('register.title')}</h2>
        ...
        <div>
          <label htmlFor="username" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('register.usernameLabel')}</label>
          <input
            type="text"
            id="username"
            value={username}
        ...
        <div>
          <label htmlFor="confirm-password" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('register.confirmPasswordLabel')}</label>
          <div className="relative mt-1">
            <input
        ...
        >
          {isLoading ? <Spinner size="sm" /> : t('register.button')}
        </button>
